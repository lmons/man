...
EOF;
    ...
    $myOwnSqlQuery="
   SELECT
 cpts.id_c,cpts.week_c,cp.id,users.first_name AS user_first_name,users.last_name AS user_last_name,cp.start_date,cpt_contacts.id,calls.name,calls.date_start,calls_contacts.call_id,contacts.first_name,contacts.last_name,cp.assigned_user_id,cp.name,cp.active,cpt.customer_tier,cp_cpt.thin_cycleplans_thin_cycleplantargetsthin_cycleplans_ida,cp_cpt.id
FROM
 thin_cycleplans AS cp
INNER JOIN thin_cycleplans_thin_cycleplantargets_c AS cp_cpt ON cp_cpt.thin_cycleplans_thin_cycleplantargetsthin_cycleplans_ida=cp.id 
INNER JOIN thin_cycleplantargets AS cpt ON cp_cpt.thin_cycleplans_thin_cycleplantargetsthin_cycleplantargets_idb=cpt.id
INNER JOIN thin_cycleplantargets_cstm AS cpts ON cpts.id_c=cpt.id
INNER JOIN thin_cycleplantargets_contacts_c AS cpt_contacts ON cpts.id_c=cpt_contacts.thin_cycleplantargets_contactsthin_cycleplantargets_idb
...
INNER JOIN users ON users.id=calls.assigned_user_id 
    WHERE cp.assigned_user_id IN ($xc'')  AND calls.assigned_user_id=cp.assigned_user_id AND calls.date_start BETWEEN cp.start_date AND cp.end_date 
...
EOF;
        ...
        while ($row = $db->fetchByAssoc($result)) {
            $x = new stdClass();
            $x->{$mod_strings['LBL_AN_ACTIVITIES_MONTH']} = date("Y", strtotime($row['date_start']))."-".date("F", strtotime($row['date_start']));
