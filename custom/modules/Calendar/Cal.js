...
CAL.dialog_save = function () {
    CAL.disable_buttons();
     console.log('test if meeting or call')
    ajaxStatus.showStatus(SUGAR.language.get('app_strings', 'LBL_SAVING'));
    console.log(CAL.get("radio_meeting").checked);
    if ((CAL.get("radio_call").checked == true) && CAL.get("tm_type_c")!==null && (CAL.get("tm_type_c").value == "")) {
      CAL.enable_buttons();
        CAL.get("tm_type_c").style.background = "red";
            setTimeout(function background() {
                CAL.get("tm_type_c").style.background = "white";
            },2000);
    //addToValidate('CalendarEditView','tm_type_c','varchar',true,SUGAR.language.get('app_strings','LBL_ACCOMPANYING'));
  //   var a=$( "#tm_type_c" ).parent();
    // a.append( "<div class='demo-box'>Demonstration Box</div>")
     CAL.get("title-cal-edit").innerHTML = "Please enter a Type.";
      return;
  }
    console.log(CAL.get("duration").value);
  if (CAL.get("duration").value == "") {
     CAL.enable_buttons();
